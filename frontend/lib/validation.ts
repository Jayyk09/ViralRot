import { ImageConfig, SingleDialogue, ValidationResult } from "./types";
 ...
 */
export function getAllReferencedFilenames(transcript: {
...
export function validateFile(
    ...
    if (!ext || !ALLOWED_IMAGE_EXTENSIONS.includes(ext)) {
        errors.push(
            `File "${filename}": Invalid format (allowed: PNG, JPG, JPEG, GIF)`,
        );
    }
    ...
    if (file.size > MAX_FILE_SIZE) {
        ...
        const sizeMB = (file.size / 1024 / 1024).toFixed(1);
        warnings.push(
            `File "${filename}": Large file (${sizeMB}MB), consider compressing`,
        );
    }
    ...
    if (file.type && !file.type.startsWith("image/")) {
        warnings.push(
            `File "${filename}": MIME type "${file.type}" is not an image type`,
        );
    }
...
export function validateFilenameMatching(
    ...
    referenced.forEach((filename) => {
        if (!uploaded.has(filename)) {
            errors.push(
                `Missing file: "${filename}" referenced in transcript but not uploaded`,
            );
        }
    });
...
export function validateBeforeUpload(
    ...
    if (!transcript.dialogue) {
        ...
        if (
            ...
            transcript.dialogue.dialogue.length === 0
        ) {
            ...
            transcript.dialogue.dialogue.forEach((line, li) => {
                ...
                if (
                    !line.speaker ||
                    !["PETER", "STEWIE"].includes(line.speaker)
                ) {
    ...
    if (totalSize > MAX_TOTAL_UPLOAD_SIZE) {
        ...
        warnings.push(
            `Total upload size (${sizeMB}MB) exceeds recommended 50MB limit`,
        );
    }
...
export function validateImportedTranscript(json: string): {
    ...
    if (
        ...
        !Array.isArray(parsed.dialogue.dialogue)
    ) {
        return {
            valid: false,
    ...
    const referencedFiles = getAllReferencedFilenames(
        parsed as { dialogue: SingleDialogue },
    );
