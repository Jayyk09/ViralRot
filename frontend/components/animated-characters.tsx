...
export function AnimatedCharacters({ mounted }: AnimatedCharactersProps) {
    ...
    useEffect(() => {
        ...
        const peterBubbleTimer = setTimeout(
            () => setShowPeterBubble(true),
            1000,
        );
        const stewieTimer = setTimeout(() => setShowStewie(true), 1500);
        const stewieBubbleTimer = setTimeout(
            () => setShowStewieBubble(true),
            2000,
        );
                ...
                <div className="relative">
                    ...
                    >
                        <SpeechBubble direction="left">
                            <span className="text-xs md:text-sm font-medium text-brainrot-brown">
                                Oh hey! Someone&apos;s here to make brainrot for
                                their already brainrotted brain!
                                <span className="text-brainrot-coral">
                                    {" "}
                                    Hehehehe
                                </span>
                            </span>
                    ...
                    {peterError ? (
                        <CharacterPlaceholder
                            name="peter"
                            className="w-64 md:w-80 lg:w-96 h-80 md:h-96 lg:h-[28rem]"
                        />
                    ) : (
                        <Image
            ...
            >
                <div className="relative">
                    ...
                    >
                        <SpeechBubble direction="right">
                            <span className="text-xs md:text-sm font-medium text-brainrot-brown">
                                ...
                                . They are our
                                <span className="text-brainrot-orange">
                                    {" "}
                                    valued customer
                                </span>
                                . Now generate that{" "}
                    ...
                    {stewieError ? (
                        <CharacterPlaceholder
                            name="stewie"
                            className="w-32 md:w-40 lg:w-48 h-44 md:h-52 lg:h-64"
                        />
                    ) : (
                        <Image
