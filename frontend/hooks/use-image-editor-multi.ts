...
export function useImageEditorMulti(
        ...
        (lineIdx: number, imageIdx: number, updates: Partial<ImageConfig>) => {
            setState((prevState) => {
                ...
                if (!line?.images?.[imageIdx]) {
                    console.error(
                        `No image at line ${lineIdx}, index ${imageIdx}`,
                    );
                    return prevState;
                }
    ...
    const removeImageFromLine = useCallback(
        (lineIdx: number, imageIdx: number) => {
            setState((prevState) => {
                const newTranscript = deepClone(prevState.transcript);
                const line = newTranscript.dialogue?.dialogue[lineIdx];
        ...
        (sourceLineIdx: number, imageIdx: number, targetLineIdx: number) => {
            setState((prevState) => {
                ...
                if (!sourceLine?.images?.[imageIdx]) {
                    console.error(
                        `No image at line ${sourceLineIdx}, index ${imageIdx}`,
                    );
                    return prevState;
                }
                ...
                for (let i = sourceLineIdx + 1; i < dialogue.length; i++) {
                    ...
                    if (line.images) {
                        line.images = line.images.filter(
                            (img: ImageConfig) => img.filename !== filename,
                        );
                        if (line.images.length === 0) {
                            delete line.images;
                        }
                ...
                if (targetLineIdx > sourceLineIdx) {
                    for (
                        let i = sourceLineIdx + 1;
                        i <= targetLineIdx && i < dialogue.length;
                        i++
                    ) {
                        const line = dialogue[i];
                        if (!line.images) {
    ...
    const calculateSpanDuration = useCallback(
        (startLineIdx: number, endLineIdx: number): number => {
            ...
            for (
                let i = startLineIdx;
                i <= endLineIdx && i < dialogue.length;
                i++
            ) {
