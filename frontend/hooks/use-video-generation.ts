...
export function useJobProgress(
    ...
    const isFailed =
        progress?.type === "error" || progress?.status === "failed";
            ...
            try {
                ...
                setApiError(
                    err instanceof Error
                        ? err
                        : new Error("Failed to start transcript generation"),
                );
                throw err;
            }
        },
...
interface UseVideoGenerationReturn {
    ...
    generate: (
        request: Omit<VideoRequest, "user_id" | "karaoke_captions">,
    ) => Promise<void>;
    /** Current job ID (if started) */
    jobId: string | null;
    /** Progress update */
...
interface UseFullVideoWorkflowReturn {
    ...
    startTranscript: (
        request: Omit<TranscriptRequest, "user_id">,
    ) => Promise<void>;
...
export function useFullVideoWorkflow(
    ...
    useEffect(() => {
        ...
        transcriptGen.isLoading,
        transcriptGen.isComplete,
        transcriptGen.transcript,
        transcriptGen.error,
    ]);
    ...
    const currentProgress =
        step === "video" ? videoGen.progress : transcriptGen.progress;
